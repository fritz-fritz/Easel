...
use thiserror::Error;
use wallspan_core::NativePixelSize;
...
mod tests {
    ...
    fn missing_file_is_reported() {
        let path = PathBuf::from("/tmp/wallspan-missing-source-does-not-exist.png");
        assert!(matches!(
            decode_still(&path),
    ...
    fn small_png_decodes() {
        let dir = std::env::temp_dir().join("wallspan-decode-test");
