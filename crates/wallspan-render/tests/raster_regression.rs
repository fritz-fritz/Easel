...
fn committed_fixture_covers_with_focal_bias() {
    ...
    let out_dir = std::env::temp_dir().join("wallspan-regression-focal");
...
fn contain_letterboxes_landscape_source() {
    let dir = std::env::temp_dir().join("wallspan-regression-contain");
...
fn oversize_synthetic_image_is_rejected() {
    let dir = std::env::temp_dir().join("wallspan-regression-oversize");
    let _ = fs::create_dir_all(&dir);
    ...
    assert!(
        matches!(err, wallspan_render::DecodeError::LimitExceeded { .. }),
...
fn multi_display_job_writes_atomic_outputs() {
    let source = fixture_path("quadrants_32.png");
    let out_dir = std::env::temp_dir().join("wallspan-regression-multi");
    let _ = fs::remove_dir_all(&out_dir);
