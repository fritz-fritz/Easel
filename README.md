...
Plasma (Easel wallpaper plugin host), and Stage 7.1–7.6 platform breadth: decode/fit/raster with
physical multi-display composition, optional perspective correction (viewer distance, eye
offset, per-display tilt/yaw; ADR 0015 / 0016; Compose Physical controls + calibration wizard;
projective stills and Plasma live UV), Compose previews, Qt display enumeration with
arrangement persistence, Plasma 6 / XFCE / GNOME / feh / Windows still apply backends, dynamic
stills on every still backend (native packages on Plasma Appearance + macOS HEIC; still-frame
poller elsewhere), local library indexing, Openverse discovery with retained provenance,
reusable profiles with schedule-driven rotation, time-of-day / solar-position dynamic still sets
...
apply-payload rasters plus selective Qt GUI smoke screenshots for review. macOS packaging
remains a later Stage 7 slice (`docs/PLATFORM_SUPPORT.md`).
